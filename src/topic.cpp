...
void Topic::execute(Client &cli, const std::string& param, const std::string& cmd, std::map<int, Client>& clients, Server& server) {
    ...
    if (new_topic.empty()) {
        ...
        if (topic.empty()) {
            ...
            cli.response(":server 333 " + cli.getNickname() + " " + channel_name + " " + channel->getTopicSetBy() + " " + to_string98(channel->getTopicTimestamp()) + "\r\n");
        }
        return;
    }
	...
	channel->setTopic(new_topic, cli.getNickname());
